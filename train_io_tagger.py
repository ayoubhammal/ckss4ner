...
cmd.add_argument("--loss", type=str, default="supervised")
...
train_data = [{"tokens": d["tokens"], "bio_tags": d["tags"], "tags": [tag if not tag.startswith("B-") else "I-" + tag[2:] for  tag in d["tags"]]} for d in train_data]
dev_data = [{"tokens": d["tokens"], "bio_tags": d["tags"], "tags": [tag if not tag.startswith("B-") else "I-" + tag[2:] for  tag in d["tags"]]} for d in dev_data]
test_data = [{"tokens": d["tokens"], "bio_tags": d["tags"], "tags": [tag if not tag.startswith("B-") else "I-" + tag[2:] for  tag in d["tags"]]} for d in test_data]
...
def eval(data, dataset_name, ostream=sys.stderr):
    ...
    for batch_data in data:
        ...
        for sentence, pred in zip(batch_data, preds):
            evaluator(sentence["bio_tags"], tag_dict.io_to_bio(tag_dict.idx_to_tags(pred)))
