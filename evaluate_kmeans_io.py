...
from ratio_constraints.utils import batchify, TagDict, project_target_transitions, get_abstract_transitions
...
def make_evaluate(disckmeans_evaluator, discKmeans_transition_evaluator, decoder, transition_weights, tag_dict, io_tag_dict, cluster2tag):
    def evaluate(kmeans, features, full_tags):
        disckmeans_evaluator.reset()
        discKmeans_transition_evaluator.reset()
        ...
        for i_batch in range(0, len(features), args.eval_minibatch_size):
            ...
            for e in batch_emissions_per_cluster:
                emission = torch.full((e.shape[0], len(io_tag_dict)), -np.inf, device=args.device)
            ...
            for e, gold_tags in zip(batch_emissions, batch_tags):
                ...
                disckmeans_evaluator(tag_dict.idx_to_tags(gold_tags), io_tag_dict.io_to_bio(io_tag_dict.idx_to_tags(predictions)))

                gold_tags_io = np.array(io_tag_dict.tags_to_idx(io_tag_dict.bio_to_io(tag_dict.idx_to_tags(gold_tags))))

                cpt += len(gold_tags)
                accuracy += (predictions == gold_tags_io).sum()
            ...
            disckmeans_transition_predictions = decoder(batch_emissions, transition_weights, batch_n_tokens)
            torch.set_grad_enabled(False)
            
            for gold_tags, pred in zip(batch_tags, disckmeans_transition_predictions):
                discKmeans_transition_evaluator(tag_dict.idx_to_tags(gold_tags), io_tag_dict.io_to_bio(io_tag_dict.idx_to_tags(pred)))

        accuracy = accuracy / cpt

        return {"disckmeans": disckmeans_evaluator.f1(), "discKmeans-transition": discKmeans_transition_evaluator.f1()}, accuracy
    return evaluate
...
pretrain_data = ratio_constraints.io.read(args.pretrain_train)
pretrain_tag_dict = TagDict(
    set(
        itertools.chain(*[sentence["tags"] for sentence in pretrain_data])
...
io_tag_dict = tag_dict.to_io_tag_dict()
...
for support_data in supports_data:
    if not args.dont_use_dev:
        ...
        for d in support_data:
            ...
            for du in data:
                if len(d["tokens"]) == len(du["tokens"]) and \
                    ...
                    found = True
                    du["tags"] = io_tag_dict.bio_to_io(d["tags"])
        ...
        for d in data:
            d["gold_tags"] = d["tags"]
            d["tags"] = io_tag_dict.bio_to_io(d["tags"])
...
# evaluators
disckmeans_evaluator = NEREvaluator()
disckmeans_transition_evaluator = NEREvaluator()

# transition matrix and mask
transition_weights, beginning_mask = project_target_transitions(io_tag_dict, get_abstract_transitions(pretrain_data), args.tau)
transition_weights = transition_weights.to(args.device)
beginning_mask = beginning_mask.to(args.device)

transition_mask = io_tag_dict.build_transition_mask(dtype=torch.float, device=args.device)
...
cluster2tag = dict()
i = 0
for tag, tag_id in io_tag_dict.tag_to_id.items():
    if tag == "O":
...
tag2clusters = {
    ...
    for i in range(len(io_tag_dict))
}
...
all_disckmeans_results = {
    "precision": [],
    "recall": [],
    "f1": [],
}
all_disckmeans_transition_results = {
    "precision": [],
    "recall": [],
    "f1": [],
}
...
for i_support, (support_data, train_support_data) in enumerate(zip(supports_data, train_supports_data)):
    ...
    if args.o_tag_ratio is not None:
        ...
        cluster_family_sizes = {
            tuple(tag2clusters[io_tag_dict.tag_to_id["O"]]): args.o_tag_ratio * n_total
        }
    else:
    ...
    evaluate = make_evaluate(disckmeans_evaluator, disckmeans_transition_evaluator, decoder, transition_weights, tag_dict, io_tag_dict, cluster2tag)
    ...
    disckmeans_results = {
        "precision": disckmeans_evaluator.precision() * 100,
        "recall": disckmeans_evaluator.recall() * 100,
        "f1": disckmeans_evaluator.f1() * 100,
    }
    disckmeans_transition_results = {
        "precision": disckmeans_transition_evaluator.precision() * 100,
        "recall": disckmeans_transition_evaluator.recall() * 100,
        "f1": disckmeans_transition_evaluator.f1() * 100,
    }
    print("disckmeans results: {}".format(disckmeans_results), file=sys.stderr, flush=True)
    print("disckmeans_transition results: {}".format(disckmeans_transition_results), file=sys.stderr, flush=True)
    print("disckmeans results: {}, disckmeans_transition results: {}".format(disckmeans_results, disckmeans_transition_results), flush=True)
    print("--------------------------------------------------------------------------\n", file=sys.stderr, flush=True)

    for k in all_disckmeans_transition_results.keys():
        all_disckmeans_transition_results[k].append(disckmeans_transition_results[k])

    for k in all_disckmeans_results.keys():
        all_disckmeans_results[k].append(disckmeans_results[k])


print("final disckmeans results over {} support sets:".format(len(train_supports_data)), flush=True)
print("    precision: {} +- {}".format(np.mean(all_disckmeans_results["precision"]), np.std(all_disckmeans_results["precision"])), flush=True)
print("    recall   : {} +- {}".format(np.mean(all_disckmeans_results["recall"]), np.std(all_disckmeans_results["recall"])), flush=True)
print("    f1       : {} +- {}".format(np.mean(all_disckmeans_results["f1"]), np.std(all_disckmeans_results["f1"])), flush=True)

print("final disckmeans_transition results over {} support sets:".format(len(train_supports_data)), flush=True)
print("    precision: {} +- {}".format(np.mean(all_disckmeans_transition_results["precision"]), np.std(all_disckmeans_transition_results["precision"])), flush=True)
print("    recall   : {} +- {}".format(np.mean(all_disckmeans_transition_results["recall"]), np.std(all_disckmeans_transition_results["recall"])), flush=True)
print("    f1       : {} +- {}".format(np.mean(all_disckmeans_transition_results["f1"]), np.std(all_disckmeans_transition_results["f1"])), flush=True)

print("{} +- {} | {} +- {}".format(np.mean(all_disckmeans_results["f1"]), np.std(all_disckmeans_results["f1"]), np.mean(all_disckmeans_transition_results["f1"]), np.std(all_disckmeans_transition_results["f1"])), flush=True)
